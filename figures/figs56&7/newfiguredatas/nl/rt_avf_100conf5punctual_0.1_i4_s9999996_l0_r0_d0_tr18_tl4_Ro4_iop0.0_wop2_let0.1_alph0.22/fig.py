...
dato=np.loadtxt('/Users/supercarlangas/schamm/epidemiology/Random_SLIRD/covid/proj_mx/covid-19_data/datas20_08_21/nl.dat')
ntotst=len(dato[:,0])
...
fig.text(0.5, 0.95, 'punctual_0.1', ha='center',fontsize=(20))
plt.subplots_adjust(left=0.1, right=0.95,top=0.95, bottom=0.13)


plt.subplot(221)
plt.plot(dat4[:,0]+22,cumsts[:,0],color = colormap(normalize(6)),marker='',
markevery=3,markersize=8,label='cumulative', linewidth=2)
# plt.plot(dat5[:,0]+22,dat5[:,1],color = colormap(normalize(1)),marker='',
# markevery=3,markersize=8,label='q1', linewidth=2)
# plt.plot(dat5[:,0]+22,dat5[:,2],color = colormap(normalize(2)),marker='',
# markevery=3,markersize=8,label='q3', linewidth=2)
# plt.plot(dat4[:,0]+22,dat4[:,8],color = colormap(normalize(8)),marker='',
# markevery=3,markersize=8,label='min. cumulative', linewidth=2)
# plt.plot(dat4[:,0]+22,dat4[:,9],color = colormap(normalize(9)),marker='',
...
plt.xlim(left=0, right=200 + 22)
plt.grid(True)


plt.subplot(222)
plt.bar(dat4[:,0]+22,dat4[:,1],color = colormap(normalize(2)),
...
plt.subplot(223)
plt.plot(dat3[:,0]+22,dat3[:,1],color = colormap(normalize(3)),label='W(t)', linewidth=2)
