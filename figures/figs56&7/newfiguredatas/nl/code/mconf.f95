...
public :: mconf, rtf, rtf_n
 ...
 function rtf(nt, ret, tsh) result(rt)
   ...
   open(unit=5, file='nl.dat', status='old')
   do j = 1 , nt
     read(5,*) d, in(j)
 ...
 function mconf(j) result (cf)
   ...
   elseif ( j > int(wpar(3,10) + wpar(2,10)).and. j<= int(wpar(3,11)) ) then
     cf = wpar(1,10)
   elseif ( j > int(wpar(3,11)) .and. j <= int(wpar(3,11) + wpar(2,11))) then
     cf = - (wpar(1,10) - wpar(1,11))/wpar(2,11) * (dfloat(j) - wpar(3,11)) + wpar(1,10)
   elseif ( j > int(wpar(3,11) + wpar(2,11)) ) then
     cf = wpar(1,11)  
   endif
 end function
