...
dato=np.loadtxt('/home/pclopez/scham/epidemia/StochasticSLIRD//code/Mexico_covid-19_datas/datas20_08_21/camp.dat')
ntotst=len(dato[:,0])
...
plt.subplot(221)
plt.plot(dat4[:,0]+49,cumsts[:,0],color = colormap(normalize(6)),label='cumulative', linewidth=2)
plt.plot(dat5[:,0]+49,dat5[:,1],color = colormap(normalize(1)),label='q1', linewidth=2)
plt.plot(dat5[:,0]+49,dat5[:,2],color = colormap(normalize(2)),label='q3', linewidth=2)
...
plt.xlim(left=0, right=540 + 49)
plt.grid(True)


plt.subplot(222)
plt.bar(dat4[:,0]+49,dat4[:,1],color = colormap(normalize(2)),label='daily cases', width=0.9)
plt.bar(dato[:,0],dato[:,1],color = colormap(normalize(3)),label='camp', width=0.7)
...
plt.subplot(223)
plt.plot(dat3[:,0]+49,dat3[:,1],color = colormap(normalize(3)),label='W(t)', linewidth=2)
...
plt.tight_layout()
figure_name = 'figcamp.eps'	# name of the figure to be saved
