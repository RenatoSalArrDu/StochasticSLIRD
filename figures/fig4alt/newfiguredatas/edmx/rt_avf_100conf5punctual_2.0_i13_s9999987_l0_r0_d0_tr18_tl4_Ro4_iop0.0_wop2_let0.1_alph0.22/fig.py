...
dato=np.loadtxt('/Users/supercarlangas/schamm/epidemiology/Random_SLIRD/covid/proj_mx/covid-19_data/datas20_08_21/edmx.dat')
ntotst=len(dato[:,0])
...
plt.subplot(221)
plt.plot(dat4[:,0]+20,cumsts[:,0],color = colormap(normalize(6)),marker='',
markevery=3,markersize=8,label='cumulative', linewidth=2)
plt.plot(dat4[:,0]+20,dat4[:,4],color = 'r',marker='',
markevery=3,markersize=8,label='I', linewidth=2)
# plt.plot(dat5[:,0]+20,dat5[:,1],color = colormap(normalize(2)),marker='',
# markevery=3,markersize=8,label='q1', linewidth=2)
# plt.plot(dat5[:,0]+20,dat5[:,2],color = colormap(normalize(2)),marker='',
# markevery=3,markersize=8,label='q3', linewidth=2)
# plt.plot(dat4[:,0]+20,dat4[:,8],color = colormap(normalize(8)),marker='',
# markevery=3,markersize=8,label='min. cumulative', linewidth=2)
# plt.plot(dat4[:,0]+20,dat4[:,9],color = colormap(normalize(9)),marker='',
...
plt.xlim(left=0, right=540 + 20)
plt.grid(True)


plt.subplot(222)
plt.bar(dat4[:,0]+20,dat4[:,1],color = colormap(normalize(2)),
label='daily cases', width=0.9)
plt.bar(dato[:,0],dato[:,1],color = colormap(normalize(3)),
label='edmx', width=0.7)
...
plt.subplot(223)
plt.plot(dat3[:,0]+20,dat3[:,1],color = colormap(normalize(3)),label='W(t)', linewidth=2)
