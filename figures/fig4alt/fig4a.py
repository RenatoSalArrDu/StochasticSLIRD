...
ax1.bar(cdmxp1[:,0]+shcdmx,cdmxp1[:,1],color = colormap2(normalize2(1)),label=r'$\bar{i}_s,\; \mathrm{CDMX}$', width=1)
ax1.bar(cdmx[:,0],cdmx[:,1],color = colormap3(normalize3(10)),label=r'$i_e,\; \mathrm{CDMX}$', width=0.6)
...
ax2.bar(jalp1[:,0]+shjal,jalp1[:,1],color = colormap2(normalize2(1)),label=r'$\bar{i}_s,\; \mathrm{Jal}$', width=1)
ax2.bar(jal[:,0],jal[:,1],color = colormap3(normalize3(10)),label=r'$i_e,\; \mathrm{Jal}$', width=0.6)
...
ax3.bar(nlp1[:,0]+shnl,nlp1[:,1],color = colormap2(normalize2(1)),label=r'$\bar{i}_s,\; \mathrm{NL}$', width=1)
ax3.bar(nl[:,0],nl[:,1],color = colormap3(normalize3(10)),label=r'$i_e,\; \mathrm{NL}$', width=0.6)
...
ax4.plot(cdmxp1[:,0]+shcdmx, cdmxcump[:,0],color = colormap2(normalize2(1)),label=r'$\bar{c}_s,\; \mathrm{CDMX}$', linewidth=1.5)
ax4.plot(cdmx[:,0],cdmxcum[:,0],color =colormap3(normalize3(10)),label=r'$c_e,\; \mathrm{CDMX}$', linewidth=1.5)
...
ax5.plot(jalp1[:,0]+shjal,jalcump[:,0],color = colormap2(normalize2(1)),label=r'$\bar{c}_s,\; \mathrm{Jal}$', linewidth=1.5)
ax5.plot(jal[:,0],jalcum[:,0],color =colormap3(normalize3(10)),label=r'$c_e,\; \mathrm{Jal}$', linewidth=1.5)
...
ax6.plot(nlp1[:,0]+shnl,nlcump[:,0],color = colormap2(normalize2(1)),label=r'$\bar{c}_s,\; \mathrm{NL}$', linewidth=1.5)
ax6.plot(nl[:,0],nlcum[:,0],color =colormap3(normalize3(10)),label=r'$c_e,\; \mathrm{NL}$', linewidth=1.5)
...
############################################################################
#
ax7.plot(cdmxp2[:,0]+shcdmx,cdmxp2[:,1],color = 'r', label=r'$W(t),\; \mathrm{CDMX}$',linewidth=0.8)
ax7.axvspan(0,shcdmx + swcdmx,color='gainsboro')
...
ax7.set_xticks(x)
ax7.grid(True)
left, bottom, width, height = [0.1, 0.21, 0.1, 0.13]
ax7s = fig.add_axes([left, bottom, width, height])
ax7s.plot(cdmxp2[:,0]+shcdmx,cdmxp2[:,1],color = 'r', label=r'$W(t),\; \mathrm{CDMX}$',linewidth=0.8)
ax7s.set_ylim(top = 5 , bottom = 0)
y2s=[1,5]
ax7s.set_yticks(y2s)
ax7s.set_xlim(right = shcdmx + swcdmx + 1, left = shcdmx-1)
ax7s.set_xticks([shcdmx,shcdmx + swcdmx])
ax7s.axvspan(0,shcdmx + swcdmx,color='gainsboro')
ax7s.tick_params(labelsize='12',labeltop=False, labelright=False, labelbottom=True)
ax7s.grid(True)

#####################
#
ax8.plot(jalp2[:,0]+shjal,jalp2[:,1],color = 'r', label=r'$W(t),\; \mathrm{Jal}$', linewidth=0.8)
ax8.axvspan(0,shjal + swjal,color='gainsboro')
...
ax8s.plot(jalp2[:,0]+shjal,jalp2[:,1],color = 'r', label=r'$W(t),\; \mathrm{Jal}$', linewidth=0.8)
ax8s.set_xlim(right = shjal + swjal + 1 , left = shjal-1)
ax8s.set_xticks([shjal,shjal + swjal])
ax8s.axvspan(0,shjal + swjal,color='gainsboro')
ax8s.tick_params(labelsize='12',labeltop=False, labelright=False, labelbottom=True)
ax8s.grid(True)

#####################
#
ax9.plot(nlp2[:,0]+shnl,nlp2[:,1],color = 'r', label=r'$W(t),\; \mathrm{NL}$', linewidth=0.8)
ax9.axvspan(0,shnl + swnl,color='gainsboro')
...
ax9.set_xticks(x)
ax9.grid(True)
left, bottom, width, height = [0.77, 0.21, 0.1, 0.13]
ax9s = fig.add_axes([left, bottom, width, height])
ax9s.plot(nlp2[:,0]+shnl,nlp2[:,1],color = 'r', label=r'$W(t),\; \mathrm{NL}$', linewidth=0.8)
ax9s.set_ylim(top = 22 , bottom = 0)
y9s=[5,20]
ax9s.set_yticks(y9s)
ax9s.set_xlim(right = shnl + swnl+1 , left = shnl-1)
ax9s.set_xticks([shnl,shnl + swnl])
ax9s.axvspan(0,shnl + swnl,color='gainsboro')
ax9s.tick_params(labelsize='12',labeltop=False, labelright=False, labelbottom=True)
ax9s.grid(True)


plt.tight_layout()
figure_name = 'fig4a.png'	# name of the figure to be saved
